...
$(document).off("click", ".btnPagarP").on("click", ".btnPagarP",function () {
  ...
  $.ajax({
      ...
      success: function (respuesta) {
          ...
          <div><h2 style="font-size: 27px;">${respuesta.simbolo_moneda}${Math.round10(respuesta.total_envio,-3)} (${respuesta.iso_moneda})</h2></div><hr>
          <strong style="font-size:17px;color:red;"><i class="fas fa-money-bill-alt"></i> Saldo a Transferir</strong>
          <div><h2 style="font-size: 27px;">${respuesta.simbolo_tasa}${Math.round10(respuesta.total_remesa,-3)} (${respuesta.iso_tasa})</h2></div>`)
          $('#monto-salida').val(Math.round10(respuesta.total_remesa,-3))
...
=============================================*/
  ...
  $('#form-pago').off('submit').on('submit',function(e){
      ...
      datos.append('monto-salida',Number.parseFloat($('#monto-salida').val()).toFixed(3))
...
for(var i = 0; i < montoRemesa.length; i++){
      ...
      arraySumaMonto.push(Number.parseFloat($(montoRemesa[i]).val()));
...
$(document).ready(function(){
    ...
    function load_notification(){
        ...
        $.ajax({
            ...
            dataType: "json",
            success: function(res) {
                $('.notipago').append('<span class="right badge badge-danger caja">'+res[0]['pagos']+'</span>')
                $('.notipago').after(`<input type="hidden" id="probandopago" value="${res[0]['pagos']}">`)
                $.ajax({
                    ...
                    dataType: "json",
                    success: function(respuesta) {
                        $('.noticredi').after('<span class="right badge badge-danger caja">'+respuesta[0]['creditos']+'</span>')
    
                        $('.totalnoti').html(parseFloat(res[0]['pagos'])+ parseFloat(respuesta[0]['creditos']))
